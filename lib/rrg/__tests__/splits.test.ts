 ...
 *
 * Reference: lib/rrg/splits.ts:130-194 + memory/feedback_register_drop_creator_must_be_platform.md
 *
 * Run via:   npm run test
 * Direct:    node --test --experimental-strip-types lib/rrg/__tests__/splits.test.ts
 */
...
test('brand_product_tiered: onChainCreator MUST be PLATFORM_WALLET', () => {
  const split = calculateSplit({
    totalUsdc:      10,
    brandId:        EXTERNAL_BRAND_ID,
    creatorWallet:  CREATOR_WALLET,
    brandWallet:    BRAND_WALLET,
    isBrandProduct: true,
    isLegacy:       false,
  });
  assert.equal(split.splitType, 'brand_product_tiered');
  ...
  assert.equal(split.brandUsdc,    9.75);
...
test('challenge_35_35_30: onChainCreator is PLATFORM_WALLET', () => {
  ...
  assert.equal(split.onChainCreator, PLATFORM_WALLET);
  assert.equal(split.creatorUsdc, 3.5);
  assert.equal(split.brandUsdc,   3.5);
  assert.equal(split.platformUsdc, 3.0);
});

test('brand_product_tiered with brandPctOverride respects override', () => {
  const split = calculateSplit({
    totalUsdc:        100,
    brandId:          EXTERNAL_BRAND_ID,
    creatorWallet:    CREATOR_WALLET,
  ...
  assert.equal(split.brandUsdc,    90);
