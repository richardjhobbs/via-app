 ...
 *
 * Transaction hashes are stored in both app_distributions.notes AND
 * app_purchases.payout_tx_hashes for dispute resolution.
 */

import { db } from '@/lib/app/db';
import { transferUsdc, getPlatformSigner, getRRGReadOnly } from '@/lib/app/contract';
import { type SplitResult, getSellerPct, PLATFORM_WALLET } from '@/lib/app/splits';
...
export async function insertDistributionAndPay(
  ...
  const splitModel = split.splitType === 'seller_product_tiered'
    ? 'tiered_brand'
    : split.splitType === 'legacy_70_30'
    ? 'fixed_legacy'
    : 'fixed_co_created';
  const sellerPctApplied = split.splitType === 'seller_product_tiered'
    ? getSellerPct(split.totalUsdc)
    : split.splitType === 'challenge_35_35_30'
    ? 35
    : split.splitType === 'legacy_70_30'
    ? 0
    : split.sellerUsdc > 0 ? parseFloat((split.sellerUsdc / split.totalUsdc * 100).toFixed(2)) : 0;

  await db.from('app_purchases')
    .update({
      split_creator_usdc:  split.creatorUsdc,
      split_brand_usdc:    split.sellerUsdc,
      split_platform_usdc: split.platformUsdc,
      brand_pct_applied:   parseFloat(sellerPctApplied.toFixed(2)),
  ...
  try {
    ...
    if (split.sellerUsdc > 0 && split.sellerWallet) {
      const tx = await transferUsdc(split.sellerWallet, split.sellerUsdc, nonce);
