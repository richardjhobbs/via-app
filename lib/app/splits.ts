 ...
 *   seller_product_tiered — brand self-listed product (tiered sliding split)
 */
 ...
 *
 * If `sellerPctOverride` is supplied (per-brand override on app_sellers), it
 * is returned directly.
 */
export function getSellerPct(priceUsdc: number, sellerPctOverride?: number | null): number {
...
export function applyCardFeeDeduction(
  ...
  const deductFrom = split.sellerUsdc > 0 ? 'brand' : 'creator';
  const adjusted = { ...split, cardFeeUsdc };
  if (deductFrom === 'brand') {
    adjusted.sellerUsdc = round2(Math.max(0, split.sellerUsdc - cardFeeUsdc));
  } else {
...
export function computeSplit(
  ...
  const sellerPct    = getSellerPct(priceUsdc, sellerPctOverride);
  const platformPct = 100 - sellerPct;
  return {
    creator:  0,
    brand:    round6(priceUsdc * sellerPct / 100),
    platform: round6(priceUsdc * platformPct / 100),
  };
}

export function calculateSplit(input: SplitInput): SplitResult {
  const { totalUsdc, sellerId, creatorWallet, sellerWallet, isSellerProduct, isLegacy, sellerPctOverride } = input;
  ...
  const creatorUsdc  = round2(totalUsdc * 0.35);
  const sellerUsdc    = round2(totalUsdc * 0.35);
  const platformUsdc = round2(totalUsdc - creatorUsdc - sellerUsdc);
  return {
