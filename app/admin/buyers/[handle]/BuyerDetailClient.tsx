    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <span className={`inline-block px-2.5 py-1 text-[10px] font-mono uppercase tracking-widest rounded ${
          buyer.public ? 'bg-sky-100 text-sky-900' : 'bg-background text-ink-2'
        }`}>
      ...
      <section>
        ...
        <div className="bg-paper border border-line rounded-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
          ...
          <Stat label="Owner email"      value={buyer.owner_email ?? '(no account)'} mono />
          <Stat label="Funding wallet"   value={buyer.wallet_address} mono />
          <Stat label="Agent wallet"     value={buyer.agent_wallet_address ?? '(not derived)'} mono />
          ...
          <div className="md:col-span-2">
            <div className="text-xs font-mono tracking-widest text-ink-3 uppercase mb-1">Delegation caps</div>
            <pre className="text-xs font-mono text-ink bg-background border border-line rounded-md p-3 overflow-auto">
              ...
              <tbody className="divide-y divide-line">
                {intents.map((i) => (
                  <tr key={i.id}>
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-ink-3">{i.resolved_at ? fmtDate(i.resolved_at) : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section>
        ...
        <div className="bg-paper border border-red-300 rounded-lg p-6 flex items-center justify-between gap-6">
          <div>
