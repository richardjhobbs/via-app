                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-3 text-sm flex-shrink-0 ml-4 items-center">
                      ...
                      <a href={`${scanBase}/address/${process.env.NEXT_PUBLIC_VIA_CONTRACT_ADDRESS}`} target="_blank" rel="noopener noreferrer" className="text-white/50 hover:text-white transition-colors font-mono">Scan ↗</a>
                    </div>
                  </div>
...
function BrandsTab() {
  ...
  const [loading,   setLoading]   = useState(true);
  const [creating,  setCreating]  = useState(false);
  const [inviting,  setInviting]  = useState<string | null>(null); // sellerId being invited
  const [msg,       setMsg]       = useState('');
  ...
  const loadMembers = useCallback(async (sellerId: string) => {
    setMembersLoading(true);
    try {
      const res = await fetch(`/api/rrg/admin/brands/${sellerId}/members`);
      const data = await res.json();
  ...
  const handleRemoveMember = async (sellerId: string, userId: string, email: string | null) => {
    ...
    const data = await res.json();
    if (res.ok) { setMemberMsg(`Removed ${email || userId} ✓`); loadMembers(sellerId); }
    else setMemberMsg(`Error: ${data.error}`);
  };

  const handleRoleToggle = async (sellerId: string, userId: string, role: string) => {
    ...
    const data = await res.json();
    if (res.ok) { setMemberMsg(`Role set to ${next} ✓`); loadMembers(sellerId); }
    else setMemberMsg(`Error: ${data.error}`);
  };

  const handleAddAdmin = async (sellerId: string) => {
    ...
    const res = await fetch('/api/rrg/admin/brands/invite', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ brand_id: sellerId, email }),
    });
    const data = await res.json();
    if (res.ok) {
      setMemberMsg(
  ...
  const handleEditSave = async (sellerId: string) => {
    setEditSaving(true);
    setMsg('');
    ...
    const res = await fetch(`/api/rrg/admin/brands/${sellerId}`, {
      method: 'PATCH',
      body: formData,
    });
    const data = await res.json();
                ...
                <>
                  ...
                  {inviting === b.id ? (
                    ...
                    <div className="border-t border-white/10 p-4 flex gap-3 flex-wrap">
                      ...
                      <a
                        href={`/admin/sellers/${b.slug}/concierge`}
                        className="px-4 py-1.5 text-sm border border-white/20 hover:border-white/50 transition-all"
                      >
                        Concierge Chat →
                      </a>
...
function DistributionsTab() {
  ...
  const splitLabel = (s: string) => {
    const labels: Record<string, string> = {
      ...
      'seller_product_tiered': 'Brand Product (Tiered)',
