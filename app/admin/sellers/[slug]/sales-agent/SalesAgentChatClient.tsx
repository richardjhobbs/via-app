...
export default function SalesAgentChatClient({ sellerId, sellerSlug, sellerName, brandHeadline, embedded = false }: Props) {
  ...
  const memViewStorageKey = `rrg.concierge.memView.${sellerSlug}`;
  ...
  const loadMemories = useCallback(async () => {
    const res = await fetch(`/api/seller/${sellerId}/concierge/memories?limit=50`);
    if (res.status === 401 || res.status === 403) {
      setAuthState('denied');
      setDeniedReason(res.status === 401 ? 'Sign in to continue.' : `You are not authorized to manage ${sellerName}.`);
      return;
    }
    if (!res.ok) {
      setAuthState('denied');
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch(`/api/seller/${sellerId}/concierge/chat`, {
        method: 'POST',
          ...
          <div className="bg-white border-t border-neutral-200 px-6 py-4">
            <div className="max-w-3xl mx-auto">
              <textarea
                ...
                onKeyDown={onKeyDown}
                placeholder={`Message ${sellerName} Concierge. Ctrl/Cmd+Enter to send.`}
                rows={3}
