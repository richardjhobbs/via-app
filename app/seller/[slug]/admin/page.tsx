...
import SellerTermsModal from '@/components/app/SellerTermsModal';
import HelpTip from '@/components/app/HelpTip';
import { brandAdmin, briefFields, voucherFields } from '@/lib/app/help-content';
import { BRAND_TC_VERSION } from '@/lib/app/terms';
import SalesAgentChatClient from '@/app/admin/sellers/[slug]/sales-agent/SalesAgentChatClient';
  ...
  return (
    <>
      ...
      <div className="px-6 py-8 max-w-5xl">
        {tab === 'submissions' && <><div className="flex items-center gap-2 mb-4"><h2 className="text-xs font-mono text-white/40 uppercase tracking-widest">Submissions</h2><HelpTip {...brandAdmin.submissions} /></div><SubmissionsTab sellerId={ctx.sellerId} /></>}
        {tab === 'products'    && <><div className="flex items-center gap-2 mb-4"><h2 className="text-xs font-mono text-white/40 uppercase tracking-widest">Products</h2><HelpTip {...brandAdmin.drops} /></div><ProductsTab sellerId={ctx.sellerId} /></>}
        {tab === 'briefs'      && <><div className="flex items-center gap-2 mb-4"><h2 className="text-xs font-mono text-white/40 uppercase tracking-widest">Briefs</h2><HelpTip {...brandAdmin.briefs} /></div><BriefsTab sellerId={ctx.sellerId} /></>}
        {tab === 'vouchers'    && <VouchersTab sellerId={ctx.sellerId} />}
        {tab === 'sales'       && <SalesTab sellerId={ctx.sellerId} />}
        {tab === 'concierge'   && (
          <>
            ...
            <p className="text-sm text-white/60 mb-4 max-w-2xl">
              Chat with {ctx.sellerName}&apos;s concierge to lock in events, promotions, stock notes, brand updates, or policies.
        ...
        {tab === 'settings'    && <><div className="flex items-center gap-2 mb-4"><h2 className="text-xs font-mono text-white/40 uppercase tracking-widest">Settings</h2><HelpTip {...brandAdmin.settings} /></div><SettingsTab sellerId={ctx.sellerId} /></>}
      </div>
    </>
  );
}
...
// ── Submissions Tab ───────────────────────────────────────────────
function SubmissionsTab({ sellerId }: { sellerId: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res  = await fetch(`/api/seller/${sellerId}/submissions`);
  ...
  const handleApprove = async (e: React.FormEvent) => {
    ...
    setMsg('');
    const res = await fetch(`/api/seller/${sellerId}/approve`, {
      method:  'POST',
  ...
  const handleReject = async (e: React.FormEvent) => {
    ...
    setMsg('');
    const res = await fetch(`/api/seller/${sellerId}/reject`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({
...
function ProductsTab({ sellerId }: { sellerId: string }) {
  ...
  const [brand,    setBrand]    = useState<SellerSettings | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    const [dropsRes, settingsRes, vouchersRes] = await Promise.all([
      ...
      fetch(`/api/seller/${sellerId}/vouchers`),
    ]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const res  = await fetch(`/api/seller/${sellerId}/products/create`, { method: 'POST', body: fd });
    const data = await res.json();
    setActing(false);
                ...
                <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      setMsg('');
                      const res = await fetch(`/api/seller/${sellerId}/products/${d.id}/files`, { method: 'DELETE' });
                      if (res.ok) {
...
// ── Briefs Tab ────────────────────────────────────────────────────
function BriefsTab({ sellerId }: { sellerId: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res  = await fetch(`/api/seller/${sellerId}/briefs`);
    const data = await res.json();
...
function VouchersTab({ sellerId }: { sellerId: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res  = await fetch(`/api/seller/${sellerId}/vouchers`);
      const data = await res.json();
...
function SalesTab({ sellerId }: { sellerId: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(`/api/seller/${sellerId}/sales`);
  ...
  const resend = async (purchaseId: string, to: 'brand' | 'buyer' | 'both') => {
    ...
    setMsg('');
    const res = await fetch(`/api/seller/${sellerId}/sales/resend-email`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
...
// ── Settings Tab ──────────────────────────────────────────────────
function SettingsTab({ sellerId }: { sellerId: string }) {
  const [brand,   setBrand]   = useState<SellerSettings | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (hasFiles) {
      ...
      res = await fetch(`/api/seller/${sellerId}/settings`, { method: 'PATCH', body: fd });
    } else {
      res = await fetch(`/api/seller/${sellerId}/settings`, {
        method:  'PATCH',
        headers: { 'Content-Type': 'application/json' },
            ...
            <div className="flex items-center gap-4">
              {(logoPreview || brand.logo_path) && (
                <div className="shrink-0 w-16 h-16 border border-white/15 overflow-hidden bg-white/5">
                  <img
                    src={logoPreview || `/api/seller/${sellerId}/image?type=logo&t=${Date.now()}`}
                    alt="Logo preview"
          ...
          <div>
            ...
            {(bannerPreview || brand.banner_path) && (
              <div className="w-full h-32 mb-3 border border-white/15 overflow-hidden bg-white/5">
                <img
                  src={bannerPreview || `/api/seller/${sellerId}/image?type=banner&t=${Date.now()}`}
                  alt="Banner preview"
                  className="w-full h-full object-cover"
                />
              </div>
