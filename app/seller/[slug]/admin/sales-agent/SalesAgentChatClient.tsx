...
export function SalesAgentChatClient({ sellerId, sellerName, sellerSlug, seedGreeting }: Props) {
  ...
  async function refreshMemories() {
    try {
      const res = await fetch(`/api/seller/${sellerId}/sales-agent/memories?limit=20`, { cache: 'no-store' });
      if (!res.ok) return;
      const json = await res.json();
      if (Array.isArray(json.memories)) setMemories(json.memories);
    } catch {
      // silent — memory panel is non-critical
    }
  }
  ...
  async function send() {
    ...
    try {
      ...
      const res = await fetch(`/api/seller/${sellerId}/sales-agent/chat`, {
        method: 'POST',
      ...
      if (json.toolCalls?.some((c: ToolCall) => c.name === 'store_seller_memory' || c.name === 'forget_seller_memory')) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-8 items-start">
      <div className="bg-white border border-neutral-200 rounded-lg shadow-sm">
          ...
          {turns.map((t, i) => (
            <div key={i} className={t.role === 'user' ? 'pl-12' : 'pr-12'}>
              <div className="text-[10px] font-mono tracking-widest uppercase text-neutral-400 mb-1">
                {t.role === 'user' ? 'You' : sellerName + ' agent'}
              </div>
        ...
        <div className="border-t border-neutral-200 p-4">
          <textarea
            ...
            onKeyDown={onKey}
            placeholder={`Tell ${sellerName} agent something to remember. Enter to send, Shift+Enter for newline.`}
            rows={3}
            disabled={sending}
      ...
      <aside className="bg-white border border-neutral-200 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-mono tracking-widest uppercase text-neutral-500">Locked-in memories</p>
          <button
            type="button"
        ...
        <p className="text-[10px] font-mono text-neutral-400 mt-4 leading-relaxed">
          Buying agents read these at <code>/sellers/{sellerSlug}/mcp</code> via the{' '}
          <code>ask_sales_agent</code> tool.
        </p>
      </aside>
    </div>
  );
