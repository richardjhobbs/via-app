...
export default function SalesAgentPreviewClient({
  sellerId,
  sellerSlug,
  sellerName,
  brandHeadline,
  ...
  const send = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/seller/${sellerId}/concierge/preview-chat`, {
        method:  'POST',
  ...
  }, [sellerId, input, messages, sending]);
      ...
      {!embedded && (
        <header className="bg-white border-b border-neutral-200 px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <a href={`/brand/${sellerSlug}`} className="text-xs text-neutral-600 hover:text-neutral-900 font-mono">
              ← {sellerName}
            </a>
          </div>
        </header>
            ...
            <div className="max-w-3xl mx-auto space-y-4">
              {messages.length === 0 && (
                <div className="text-neutral-500 text-sm border border-dashed border-neutral-300 rounded p-4 leading-relaxed">
                  Ask the {sellerName} Concierge anything a shopper might ask: returns,
          ...
          <div className="bg-white border-t border-neutral-200 px-6 py-4">
            <div className="max-w-3xl mx-auto">
              <textarea
                ...
                onKeyDown={onKeyDown}
                placeholder={`Ask the ${sellerName} Concierge anything. Ctrl/Cmd+Enter to send.`}
                rows={3}
