...
import { z } from 'zod';
import {
  db, getApprovedListings, getCurrentBrief, getListingByTokenId,
...
function createRRGServer() {
  const server = new McpServer(
    ...
    },
    {
      instructions: [
        ...
        '',
        '**Listings** (`list_drops` or `list_listings`) = finished products available to BUY. NFT collectibles, physical items, digital assets.',
        ...
        'If asked what is for sale or to buy something → call `list_drops` (or `list_listings`).',
        '',
        ...
        'You will NOT receive an email confirmation — check via list_drops (or list_listings) or get_drop_details (or get_listing_details) after approval.',
        '',
        ...
        '',
        '1. Call `list_drops` (or `list_listings`) to see all available listings. Note the tokenId and priceUsdc of the listing you want.',
        ...
        '1. GET /api/rrg/listing/{tokenId}/content → 402 with payment details.',
  ...
  // ── Tool: list_listings (alias for list_drops) ───────────────────────────
  server.tool(
    'list_listings',
    '[BROWSE] Alias for list_drops. List all active NFT listings available for purchase. START HERE to see what is for sale. Optionally filter by brand_slug. Returns title, price in USDC, edition size, remaining supply, and revenue split. Next step: call initiate_agent_purchase to buy (AI agents must use this, not initiate_purchase).',
    {
      brand_slug: z.string().optional().describe('Optional brand slug to filter listings by a specific brand'),
    },
    async ({ brand_slug }) => {
        ...
        content: [{
          ...
          text: JSON.stringify({
            ...
            message:
              ...
              'If approved, your design will be listed as an NFT listing at https://realrealgenuine.com/rrg. ' +
  ...
  server.tool(
    ...
    async ({ submission_id }) => {
      ...
      let listingInfo: { tokenId?: number; listingUrl?: string } = {};
      if (data.status === 'approved') {
        ...
        if (drop?.token_id) {
          listingInfo = {
            tokenId: drop.token_id,
            listingUrl: `https://realrealgenuine.com/rrg/listing/${drop.token_id}`,
          };
        }
      }
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            ...(data.status === 'approved' && listingInfo),
  ...
  server.tool(
    'list_brands',
    '[BROWSE] List all active brands on the platform. Returns name, slug, headline, description, and product/brief counts. Use a brand slug with list_drops (or list_listings) or list_briefs to filter by brand.',
    {},
    async () => {
  ...
  // ── Tool: get_drop_details (alias: get_listing_details) ─────────────────
  server.tool(
    'get_drop_details',
    [
      '[BROWSE] Get full details for a specific listing by tokenId. Call this after list_drops (or list_listings) to see what you are buying.',
      ...
      'Alias: get_listing_details — both tools are identical.',
    ].join('\n'),
    {
      tokenId: z.number().int().positive().describe('Token ID of the listing'),
    },
  ...
  // ── Tool: get_listing_details (alias for get_drop_details) ───────────────
  server.tool(
    'get_listing_details',
    [
      '[BROWSE] Alias for get_drop_details. Get full details for a specific listing by tokenId. Call this after list_listings (or list_drops) to see what you are buying.',
      'Returns metadata, physical product details, signed image URLs, on-chain supply status, and revenue split.',
      'Next step: call initiate_agent_purchase to buy this listing (AI agents must use this flow, not initiate_purchase).',
      'Alias: get_drop_details — both tools are identical.',
    ].join('\n'),
    {
      tokenId: z.number().int().positive().describe('Token ID of the listing'),
    },
        ...
        content: [{
          ...
          text: JSON.stringify({
            ...
            channels: {
              ...
              drops:         { name: '#rr-drops',         id: '1482200038896828678', purpose: 'Listing notifications & commerce alerts' },
            },
            communityFocus: [
  ...
  server.tool(
    'redeem_voucher',
    [
      '[AFTER PURCHASE] Redeem a voucher code (RRG-XXXX-XXXX) received after buying a listing.',
  ...
  server.tool(
    ...
    {
      tokenId:     z.number().int().positive().describe('The token ID of the listing to purchase'),
      buyerWallet: z.string().regex(/^0x[0-9a-fA-F]{40}$/).describe('Your wallet address on Base'),
    },
    async ({ tokenId, buyerWallet }) => {
      const drop = await getListingByTokenId(tokenId);
...
export async function GET(req: Request) {
  ...
  if (accept.includes('application/json') || !accept.includes('text/html')) {
    ...
    return Response.json({
      ...
      mcp_tools: [
        { name: 'list_drops',              description: 'Browse all active listings. Optional filter { brand_slug: string }. Alias: list_listings.' },
        { name: 'list_listings',           description: 'Alias for list_drops. Browse all active listings. Optional filter { brand_slug: string }.' },
        { name: 'get_drop_details',        description: 'Full details for one listing by tokenId. Alias: get_listing_details.' },
        { name: 'get_listing_details',     description: 'Alias for get_drop_details. Full details for one listing by tokenId.' },
