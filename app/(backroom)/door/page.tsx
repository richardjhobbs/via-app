...
export default async function DoorPage({ searchParams }: { searchParams: Promise<{ ref?: string; handle?: string }> }) {
  const sp = await searchParams;
  let handle = (typeof sp.ref === 'string' ? sp.ref : typeof sp.handle === 'string' ? sp.handle : '').trim();
  if (!handle) {
