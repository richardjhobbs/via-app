...
// POST /api/seller/auth/forgot-password : send password reset email
export async function POST(req: NextRequest) {
  try {
    if (isRateLimited(`seller-forgot|${clientIp(req)}`, 5, 60_000)) {
    ...
    const { data: link, error } = await supabaseAdmin.auth.admin.generateLink({
      type: 'recovery',
      email,
      options: { redirectTo: `${siteUrl}/seller/login?reset=true` },
    });
    const actionLink = link?.properties?.action_link;
    if (error || !actionLink) {
      console.error('[seller/forgot-password] generateLink', error);
    } else {
      try {
        await sendPasswordResetEmail({ to: email, url: actionLink });
      } catch (sendErr) {
        console.error('[seller/forgot-password] send', sendErr);
      }
    }
    ...
    console.error('[/api/seller/auth/forgot-password]', err);
