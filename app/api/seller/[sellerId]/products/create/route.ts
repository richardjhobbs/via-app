...
import { db, claimNextTokenId, getBrandById, getCurrentNetwork, RRG_BRAND_ID } from '@/lib/app/db';
import { getRRGContract, toUsdc6dp } from '@/lib/app/contract';
import { uploadSubmissionFile, jpegStoragePath, additionalFileStoragePath, additionalFilesPath, physicalImageStoragePath } from '@/lib/app/storage';
import { calculateSplit } from '@/lib/app/splits';
import { autopostApproval } from '@/lib/app/autopost';
import { getSignedUrl } from '@/lib/app/storage';
import { analyzeBrandImageQuality } from '@/lib/app/vision';
import { randomUUID } from 'crypto';
import { isValidShippingRegion } from '@/lib/app/physical-product';
import type { ShippingType } from '@/lib/app/db';
...
// POST /api/seller/[sellerId]/products/create — brand self-lists a product
export async function POST(
  ...
  try {
    ...
    const split = calculateSplit({
      ...
      sellerWallet:      brand.wallet_address,
    ...
    console.error('[/api/seller/[sellerId]/products/create]', err);
