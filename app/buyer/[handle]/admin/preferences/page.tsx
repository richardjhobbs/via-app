      ...
      <section className="flex-1 px-6 py-12">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-sm text-neutral-600 mb-8">
            ...
            <Link href={`/buyer/${handle}/admin/buying-agent`} className="underline hover:text-neutral-900">
              training chat
            </Link>.
          </p>
          ...
          {rows.length === 0 ? (
            ...
            <ul className="space-y-3">
              {rows.map((m) => (
                <li key={m.id} className="bg-white border border-neutral-200 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] font-mono uppercase tracking-widest px-2 py-0.5 rounded bg-neutral-100 text-neutral-500">
                      {m.type}
                    </span>
                    {Array.isArray(m.tags) && m.tags.length > 0 && (
                      <span className="text-[10px] font-mono text-neutral-400">{m.tags.join(', ')}</span>
                    )}
                  </div>
                  <div className="text-sm font-medium text-neutral-900 mb-1">{m.title}</div>
                  <div className="text-sm text-neutral-600 leading-relaxed">{m.body}</div>
                </li>
              ))}
            </ul>
          )}
