...
const BRANDS = {
  ...
  'nolo': {
    slug:            'nolo',
    name:            'Nolo',
    // Nolo brand agent, wallet minted 2026-04-25 for Pierre handover, ERC-8004 agent #45690
    // (Superseded original wallet 0x891C13aA..., agent #45040 from 2026-04-17.)
    wallet:          '0x27daa49fB93445cDB6e3f3a6BE7Cd6baE1f04E2d',
    email:           'pierre@wearenolo.com',
  ...
  'howies': {
    ...
    wallet:          '0x734a25fB869ab6415b78bbe9a39f1f99dab349E7',
    email:           'richard@entrepot.asia',
    headline:        'Conscious clothing from a quiet corner of Wales.',
    description:     'howies is a small, employee-owned active clothing company based in Carmarthen, Wales, founded in 1995. The brand makes lower-impact clothing for the outdoors, running and cycling alongside everyday functional wear, using organic, recycled or natural fabrics wherever possible: GOTS-certified organic cotton across t-shirts, jeans and sweats, and high-quality merino wool across the sport base layers. Independent again since 2011, with a track record of firsts: organic Ventile in 2010, fluorocarbon-free in 2016, single-use plastic eliminated from packaging in 2019, compostable corn-starch packaging in 2022. Mirror of howies.co.uk, GBP priced into USDC on Base, ships from howies HQ in Wales.',
    website:         'https://howies.co.uk',
    shopifyDomain:   'howies.co.uk',
    supportsSizing:  true,
    sourceCurrency:  'GBP',
    priceToUsdcRate: 1.35,
    socialLinks:     { instagram: 'https://www.instagram.com/howies/' },
    bannerLocal:     null,
    logoLocal:       null,
  },
  'stadium-goods': {
...
async function importProduct(product, brand) {
  ...
  // PLATFORM_WALLET, never the brand wallet. RRG.sol mintWithPermit
  // hard-codes a 70% atomic transfer to drop.creator. The off-chain
  // auto-payout (lib/rrg/auto-payout.ts) tops the brand up to its
  // negotiated split (typically 97.5%) from PLATFORM reserves and
  // does not reclaim the on-chain leg. If drop.creator = brand wallet,
  // the brand gets 70% on-chain + 97.5% off-chain (= 167.5% paid out)
  // and the platform takes a 67.5% loss per sale.
  //
  // Canonical pattern: lib/rrg/splits.ts:160 returns
  // onChainCreator: PLATFORM_WALLET for brand_product_tiered. The
  // brand-product create flow at app/api/brand/[brandId]/products/create/
  // route.ts:280 uses calculateSplit().onChainCreator. This script
  // mirrors that contract.
  //
  // Post-mortem: feedback_register_drop_creator_must_be_platform.md
  if (!SKIP_CHAIN) {
    ...
    console.log(`  → registerDrop(${tokenId}, ${PLATFORM_WALLET}, ${toUsdc6dp(price)}, ${CFG.fixedEdition})  [nonce=${nonce}]`);
