...
(async () => {
  ...
  console.log(`     (a) ui_visible=true rows (the curated storefront shelf): MUST be image-enhanced. Download each row's`);
  console.log(`         jpeg_storage_path + physical_images_paths from Supabase Storage and VIEW THE IMAGES before`);
  console.log(`         writing enhanced_description + product_attributes. The storefront cards represent the brand,`);
  console.log(`         so prose must reflect what is actually in the image (colour, glaze, motif, shade fabric,`);
  console.log(`         construction detail), not a regex extraction of the Shopify body.`);
  console.log(`     (b) ui_visible=false rows (MCP-only long tail): basic text-only enhancement from the Shopify body`);
  console.log(`         is acceptable for now. Image-enhancement of the full catalogue is a post-confirmation upgrade,`);
  console.log(`         not a Stage 1 cost. Note the unenhanced rows in the brand's Notion Integration page.`);
  ...
  console.log(`     under parent 34ddbc7b67f2809bb287c81c4493ba7f ("Brand Integration"), then customise the brand name,`);
  console.log(`     slug, currency, FX, products table, and storefront link.`);
